...
describe('Header', () => {
    ...
    it('renders the header with additional styles when scrolled', () => {
        ...
        expect(headerComponent).toHaveClass('flex items-center justify-center z-10 p-2 bg-[#F9BB38] text-[#9B4428] drop-shadow-2xl fixed top-0 left-0 right-0 font-bold shadow-md shadow-black-300');
        expect(headerElement).toHaveClass('text-center bg-[#F9BB38] text-2xl sm:text-4xl hover:cursor-pointer');
        expect(headerElement).toBeInTheDocument();
        expect(linkElement).toBeInTheDocument();
        expect(linkElement).toHaveAttribute('href', '/');
